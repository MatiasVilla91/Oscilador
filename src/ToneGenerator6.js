...
function ToneGenerator() {
  const [frequencies, setFrequencies] = useState({
    // Estado para las frecuencias de los osciladores...
  });
  const [isPlaying, setIsPlaying] = useState(false);
  const [waveform, setWaveform] = useState('sine');
  const [activeNotes, setActiveNotes] = useState({
    // Estado para las notas activas de los osciladores...
  });
  ...
  const handleSequencer = (tone) => {
    const baseFrequency = frequencies[tone];
    const { note } = getNoteAndFrequency(baseFrequency);
    const thirdAboveFrequency = frequencyList[note + '3']; // Obtener la tercera mayor
    const thirdAboveNote = getNoteAndFrequency(thirdAboveFrequency).note;

    setFrequencies(prevFrequencies => ({
      ...prevFrequencies,
      [tone]: thirdAboveFrequency,
    }));
    setActiveNotes(prevActiveNotes => ({
      ...prevActiveNotes,
      [tone]: thirdAboveNote,
  ...
  return (
    <div>
      ...
      {/* Aquí va el JSX para los controles de frecuencia y demás... */}

      <button onClick={() => handleSequencer('oscillator1')}>Activar Secuenciador</button>
      {/* Repite el botón para cada oscilador... */}
